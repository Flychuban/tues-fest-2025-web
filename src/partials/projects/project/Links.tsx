...
const Linky = ({ text, url, icon }: { text: string; url: string; icon: ReactNode }) => (
	<Link
		...
		target="_blank"
		rel="noreferrer"
		className="hover:bg-bg-color-hover border-border hover:bg-bg-color flex w-full items-center gap-2 rounded-xl border-2 bg-black px-4 py-2"
	>
		{icon}
...
const LinksContainer = ({ links }: { links: Links }) => (
	<div className="m-auto mx-auto mt-4 w-[96%] text-white opacity-100 md:w-[90%] lg:w-[70%]">
		<div className=" border-stroke  rounded-xl border-2 bg-black">
